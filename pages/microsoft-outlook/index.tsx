...
} from '../../utils/microsoft-outlook/actions'
import {
  reducer,
  initialState,
  FetchingStage,
} from '../../utils/microsoft-outlook/reducer'
...
export default function MicrosoftOutlookPage(props: Props) {
  const { emails } = props
      ...
      <Overlay>
        <Loading>
          ...
          {state.fetchingStage === FetchingStage.fetchingMessages
            ? `Loading ${state.messagesCount} messages...`
            : null}
          {state.fetchingStage === FetchingStage.signingFile
            ? `Signing metadata file to upload...`
            : null}
          {state.fetchingStage === FetchingStage.uploadingFile
            ? `Uploading metadata file...`
            : null}
          {state.fetchingStage === FetchingStage.notifyApp
            ? `Notify application about upload...`
            : null}
        </Loading>
      </Overlay>
    )
